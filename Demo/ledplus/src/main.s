...
mov_right:
    lsr r0, #0           // Shift Leds right
    b mov_done
mov_left:
    lsl r0, #0           // Shift Leds left
    b mov_done
mov_done:
...
check_button_change:
    ...
    movs r3, #0
    movs r0, #0x18   // write 0b'000011 to Leds
    ldr r5, =Leds   // r5 = Leds addr
    strb r0, [r5]         // Last button status is 0
...
.text
    .global main
    .equ sec, 0x16E360       // 150,0000
    .equ half_sec, 0xB71B0   // 75,0000
    .equ mini_sec,  0x96     // 150
    .include "../src/pin.s"  // import GPIO constants
...
Delay:
    ...
    ldr r0, =mini_sec        // Use to control total delay time
Delay_loop:
    subs r0, r0, #1          // 1 cycles
    bne Delay_loop           // 1~3 cycles
    POP {r0, pc}

// Delay half a second
Delay_half:
    PUSH {r0, lr}
    ldr r0, =half_sec        // Use to control total delay time
Delay_half_loop:
    subs r0, r0, #1          // 1 cycles
    bne Delay_half_loop      // 1~3 cycles
    POP {r0, pc}
...
check_pass:
    PUSH {r0, r1, lr}
    ldr r1, =GPIOA_IDR     // Check the tried password
    ldr r0, [r1]
    eor r0, #0xFFFFFFFF    // Reverse bits due to active low
    and r0, #0x3C0         // Only grab the bits we need
    lsr r0, r0, #6
    ldr r1, =password      // Load the real password
    ldr r1, [r1]
    eor r0, r0, r1         // use XOR to check for umatch bits
    cbz r0, check_correct  // Check if there are unmatched bits
    bl Blink_LED           // Blink for 1 time
    POP {r0, r1, pc}
check_correct:
    bl Blink_LED           // Blink for 3 times
    bl Blink_LED
    bl Blink_LED
    POP {r0, r1, pc}
...
Blink_LED:
    PUSH {r0, lr}
    mov r0, #0x78  // Set up r0 = 0b'1111000
    bl DisplayLED  // Change the LEDs
    bl Delay_half  // Delay for half a sec
    mov r0, #0x0   // Set up r0 = 0b'0000000
    bl DisplayLED  // Change the LEDs
    bl Delay_half  // Delay for half a sec
    POP {r0, pc}
...
check_button:
    PUSH {r0, r1, r2, lr}
    movs r0, #100          // Check for 100 times for debounce
    movs r2, #0            // r2 = The amount of times button equals 1
check_button_loop:
    bl Delay               // Delay for a short period of time
    ldr r1, =GPIOC_IDR     // Check if button is pressed
    ldr r1, [r1]
    ands r1, #0x2000
    bne check_button_end   // If button = 0, branch to check_button_end
    add r2, r2, #1         // Else, increment button press counter
check_button_end:
    subs r0, r0, #1        // Decrement counter
    bne check_button_loop
    cmp r2, #50            // Check if there are more than 50 ones
    blt check_button_check // Negative edge trigger
    movs r3, #1            // Last button status is 1
    b check_button_nocheck
check_button_check:
    cbz r3, check_button_nocheck  // Check if switch from 1 to 0
    movs r3, #0            // Last button status is 0
    bl check_pass          // Check password
check_button_nocheck:
    POP {r0, r1, r2, pc}
...
main:
    bl GPIO_init           // Setup GPIO
    mov r0, #0x0           // Inital LEDs are not lit
    bl DisplayLED
    mov r3, #0             // Inital button is not pressed
    b Loop
